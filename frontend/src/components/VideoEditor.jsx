...
export function VideoEditor() {
  ...
  const activeMedia = activeMediaId
    ? mediaFiles.find((m) => m.id === activeMediaId)
    : null;
  ...
  // const handleExport = () => {
  //   if (!activeMedia) {
  //     showToastNotification("No media selected for export");
  //     return;
  //   }
  
  //   // Gather export options: original media (option 0) + available versions
  //   const versions = filteredVersions;
  //   const options = [
  //     { id: "original", label: `Original (${activeMedia.file.name})`, url: activeMedia.url },
  //     ...versions.map((v) => ({
  //       id: v.id,
  //       label: `${v.versionNumber} (${formatDate(new Date(v.timestamp))})`,
  //       url: v.url,
  //     })),
  //   ];
  ...
  //       options.map((opt, index) => `${index}: ${opt.label}`).join("\n")
  //   );
  
  //   const choiceNumber = parseInt(choice, 10);
  //   if (isNaN(choiceNumber) || choiceNumber < 0 || choiceNumber >= options.length) {
  //     showToastNotification("Invalid choice");
  //     return;
  //   }
  
  //   // Get the selected version URL
  //   const selectedVersion = options[choiceNumber];
  //   if (!selectedVersion.url) {
  //     showToastNotification("Invalid selection");
  //     return;
  //   }
  
  //   // Create a temporary anchor element to trigger file download
  //   const anchor = document.createElement("a");
  //   anchor.href = selectedVersion.url;
  //   anchor.download = selectedVersion.url.split("/").pop();
  //   document.body.appendChild(anchor);
  //   anchor.click();
  //   document.body.removeChild(anchor);
  
  //   showToastNotification(`Exporting: ${selectedVersion.label}`);
  // };
    ...
    <div className={`video-editor ${isDarkMode ? "dark-mode" : ""}`}>
      ...
      <div className="sidebar left-sidebar">
        ...
        <div className="upload-section">
          ...
          {/* <button className="button primary export-button" onClick={handleExport}> 
            <Download size={16} />
            Export Media
          </button> */}
        </div>
        ...
        <div className="editor-content">
          {activeMedia ? (
            ...
            <div className="empty-preview">
              ...
              <p>
                Upload or select a file from your library to preview it here
              </p>
            </div>
          )}
          ...
          <div className="prompt-section" style={{ position: "relative" }}>
            <div className="form-group">
              ...
              <p className="prompt-hint">
                Tip: Use @ to mention a specific file. Press Ctrl+Enter to
                submit.
              </p>
            </div>
            <button
          ...
          {!activeMediaId ? (
            ...
            <div className="empty-state">
              ...
              <p className="subtitle">
                Use the prompt to create edited versions
              </p>
            </div>
          ) : (
